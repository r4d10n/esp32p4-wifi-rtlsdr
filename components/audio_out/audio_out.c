/*
 * I2S Audio Output — ES8311 Codec via esp_codec_dev
 *
 * Waveshare ESP32-P4-WIFI6 board:
 *   ES8311 codec on I2C (0x18) + I2S0
 *   NS4150B Class-D amp on GPIO53
 *
 * Uses esp_codec_dev abstraction (same as official ESP-IDF ES8311 example).
...
esp_err_t audio_out_init(const audio_out_config_t *config)
{
    ...
    /* 2. Initialize I2S TX channel — always STEREO (ES8311 expects it) */
    ...
    /* 4. Initialize speaker amp GPIO (esp_codec_dev may already handle PA via pa_pin,
     * but we also manage it explicitly for mute control) */
    pa_ctrl_init();
...
{
    if (!s_i2s_tx || !samples || frames <= 0) return ESP_ERR_INVALID_STATE;

    /* samples is already interleaved L/R, write directly */
    size_t bytes_written = 0;
    size_t total_bytes = frames * 2 * sizeof(int16_t);
    ...
    esp_err_t ret = i2s_channel_write(s_i2s_tx, samples, total_bytes,
                                       &bytes_written, timeout_ms);
    if (ret != ESP_OK && ret != ESP_ERR_TIMEOUT) return ret;
    return ESP_OK;
}
...
{
    /* I2S always runs in stereo mode for ES8311 compatibility.
     * Stereo vs mono is handled in audio_out_write() by
     * either duplicating mono or passing through stereo. */
    (void)stereo;
    return ESP_OK;
}
